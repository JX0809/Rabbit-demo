    ...
    if (sku.inventory) {
      ...
      powerSet.forEach((set) => {
        ...
        if (pathMap[key]) {
           ...
           * 给对象添加新属性  obj.name ='zs'  huo  obj['age'] = '18'
           * huo  const key = 'hobby'  obj[key] = 'singing'
           * obj = {name: 'zs', age='18', hobby: 'singing'}
           */
        }
      })
    }
  ...
  setup(props, { emit }) {
    ...
    const selectItem = (item, val) => {
      ...
      if (selectedArr.length === props.goodsInfo.specs.length) {
        // 在路径字典里找到 用户选中的这一项 ： 路径字典存储了sku.id
